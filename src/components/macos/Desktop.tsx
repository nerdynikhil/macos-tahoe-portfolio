...
function DesktopContent() {
  ...
  return (
    <div className={isDarkMode ? "dark" : ""} style={{ fontFamily: FONT_FAMILY }}>
      <div
      ...
      >
        <MenuBar />
        <div className="absolute inset-0 top-7 bottom-0" onClick={handleDesktopClick}>
          {openWindows.map((win) => {
            let content: React.ReactNode = null
            if (win.id === "finder") content = <FinderContent />
            else if (win.id === "terminal") content = <TerminalContent />
            else if (win.id === "safari") content = <SafariContent />
